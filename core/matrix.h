...
template <typename T, size_t N> struct slice_iterator {
public:
  ...
  slice_iterator(const matrix_slice<N> &s, T *base, bool end_iter = false)
    ...
    if (end_iter) {
      ...
      ptr = base + s.start + desc.extents[0] * desc.strides[0];
    } else {
      ptr = base + s.start;
    }
  }
...
operator==(const M1 &a, const M2 &b) {
  ...
  static_assert(
      ...
      "Incompatible type");
  static_assert(std::is_integral<typename M1::value_type>::value,
...
template <typename T> using matrix1 = matrix<T, 1>;
